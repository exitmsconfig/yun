...
cat nagios-09-18-2018-00.log  |grep mail  |grep Warning |grep em |grep Traffic > 1.log
cat nagios-09-18-2018-00.log  |grep mail  |grep Warning |grep em |grep Traffic | awk -F '[;]' '{print $2}' >2.log
sort 2.log | uniq -c | sort -n > 3.log

#查nagios告警Critical太多IP,并排序重复次数
cat nagios-09-18-2018-00.log  |grep mail  |grep Critical |grep em |grep Traffic > 1.log
cat nagios-09-18-2018-00.log  |grep mail  |grep Critical |grep em |grep Traffic | awk -F '[;]' '{print $2}' >2.log
sort 2.log | uniq -c | sort -n > 3.log

#查nagios全部告警太多IP,并排序重复次数
cat nagios-09-18-2018-00.log  |grep mail  > 1.log
cat nagios-09-18-2018-00.log  |grep mail  | awk -F '[;]' '{print $2}' >2.log
sort 2.log | uniq -c | sort -n > 3.log
...
20180320 目录
    10.1.0.1_20180320*.log.gz   目录
        201803200000.log.gz          文件
        201803200010.log.gz          文件
    10.1.0.2_20180320*.log.gz   目录
        201803200000.log.gz         文件
        201803200010.log.gz         文件

需求：去掉目录中的*.log.gz，这样很容易让人误解为文件。 rename -n为测试，rename使用和sed相同的语法。

$ for d in 201803??;do echo ${d}; cd ${d}; rename -n 's/*.log.gz//' *.log.gz ; cd ..;done
